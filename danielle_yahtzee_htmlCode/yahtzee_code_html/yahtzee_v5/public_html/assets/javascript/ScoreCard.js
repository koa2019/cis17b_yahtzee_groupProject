...
function ScoreCard() {
    ...
    this.upperScore = 0;
    this.lowerScore = 0;
    this.wasPicked = [NUM_CATGRY]; // Stops categories from being overwritten or displayed twice
...
ScoreCard.prototype.recordScore = function (category, score) {
    ...
    else if (this.round > 12 && (this.wasPicked[category - 1] === false)) {
        this.scores[indx] = score;
        this.writeAScore(indx); // rewrite points on ScoreCard.html
...
//        buttons[i].addEventListener("click", this.catgyOnClick);
//    } 
};
...
ScoreCard.prototype.isFound = function (indx) {
    console.log("   Hit isFound(). dice " + indx + " is in this.wasPicked[ ]");
    //var i = (indx === wasPicked[indx]) ? indx : undefined;
    return indx >= 0;
};
...
ScoreCard.prototype.setWasPicked = function (i) {
    this.wasPicked[i] = true;
    console.log("wasPicked[" + i + "] = "+ this.wasPicked[i]);
};
