...
/******** Read user's input and create a new profile for them  ***************/
void User::createProfile(){
                        
    bool isNameGood  = isName();
    bool isEmailGood = isEmail();
    bool isPwrdGood  = isPwrd();
    int num=13;
    
    // If user's input was valid, then create a new profile for them
    if(isNameGood && isEmailGood && isPwrdGood){

        wrtTxt(num++);  // Write to text file
...
bool User::isName(){
    ...
    if(isLeng){
        //name = toLowerCase(n);     // set variable in class    
        setName(toLowerCase(n));     // changes to lowercase & set variable in class    
...
bool User::isEmail(){
    ...
    if(isEmail && isLeng){
        //email = toLowerCase(em);   // set variable in class
        setEmail(toLowerCase(em));   // set variable in class
...
//User User::*find1Record(int recIndx){
void User::find1Record(int recIndx){ 
    ...
    // Accumlate all the bits between the beginning of file up the record I'm looking for.
    while(count<=recIndx){ 
