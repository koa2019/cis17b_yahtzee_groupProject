...
void Admin::adminPortal(){
    ...
    do {
        ...
        switch(choice){
            ...
            {    
//                // Create an array of Admin objects
                //int nRecords = user.getNumRec();
//                Admin **usrArr = nullptr;
//                usrArr = new Admin*[nRecords];
//                
//                for(int i=0; i < nRecords;i++){
//                    usrArr[i] = new Admin(i); 
//                }
                readAllUsrs();
                //readAllUsrs(usrArr, nRecords);
                //usrArr = readAllUsrs(nRecords);
                
                // Print all user's in binary file
                for(int i=0; i < totalRec;i++){
                    usrArr[i]->printAdUsr();
                }                                           
                break;
...
//Admin Admin::**readAllUsrs(int nRec){
//void Admin::readAllUsrs(Admin **usrArr, int nRec){
...
bool Admin::findByEmail(string emai){
    ...
    while( (!foundEm) && (count <= nRec) ){                 
        ...
        // Read in sizeof ttlRec and add to cursor
        int num;
        inBin.read(reinterpret_cast<char *>(&num) , sizeof(int)); 
        thisSum += sizeof(num);
        cursor  += sizeof(num);          
