...
ScoreCard.prototype.recordScore = function (category, score) {
    ...
    if ((this.isCatgryPicked[indx] === false) && this.getRound()>= 0 && this.getRound()<= 12) {
...
ScoreCard.prototype.setFinalSC = function (scorecard) {
    ...
    do {
        ...
        category = prompt(string); //cin >> choice;    
        ...
        if( (this.isCatgryPicked[category]) === true) { 
            var str2 = "Category " + (category+1) + " has already been selected.\n";
    ...
    console.log('finalSC = ' + this.isCatgryPicked + "\nscorecard = " + scorecard.isCatgryPicked);
    //console.log("\nInside setFinalSC(). category = " + category +"\nisCatgryPicked = " + this.isCatgryPicked[category]+'\nfinalSC.scores = '+ this.scores);
...
ScoreCard.prototype.reRoll = function () {
    ...
    console.log("\nHit reRoll().  this.selected dice = " + this.selected+"\n");
    //alert("Hit reRoll().  this.selected dice = " + this.selected);
    
    // Re-Roll Dice. loops through selected vector, and checks which this.dice # to re-roll ?
    for (var i = 0; i < NUM_DICE; i++) {
       
        // Javascript syntax: array.find(function(currentValue, index, arr),thisValue)
        //if (find(selected.begin(), selected.end(), j) == selected.end()) {
            console.log("...Re-Rolling Dice");
            this.roll(i); // re-roll this this.dice
        //}
    }
    this.dice.sort();
...
ScoreCard.prototype.writeScoreCard = function () {
    for (var i = 0; i < NUM_CATGRY; i++) {
        if(this.round===NUM_CATGRY && this.scores[i]=== -1){ this.scores[i] = 0;}
