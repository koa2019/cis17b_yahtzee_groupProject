...
//Function Prototypes
//bool play(Score *,Player **,int,int &);
bool play(Score *,Player **,int,int &);  //returns if player's guess was correct or not
...
int main(int argc, char** argv) {
    ...
    for(int r=0;r<maxRnd;r++){
        ...
        // Player 1's turn                       
        p1_crrt = play(&score,player,indx[0],round);
        ...
        if(p1_crrt){ 
            ...
            pause();
            p2_crrt = play(&score,player,indx[1],round); 
        }                           
...
bool play(Score *score, Player **player, int a, int &round){
    ...
    do {
        ...
        score->decreRolls(); // decrease # of rolls
